...
@RestController @RequestMapping(value = "/api/command") public class CommandController {
	...
	private AuditEntry getAuditEntry() {
		return new AuditEntry(getCurrentUser(), Calendar.getInstance().getTime());
	}

	@Autowired public CommandController(CommandGateway commandGateway) {
		this.commandGateway = commandGateway;
	}

	@RequestMapping(value = "/customer/createcommand", method = RequestMethod.POST, consumes = MediaType.APPLICATION_JSON_VALUE) @ResponseStatus(value = HttpStatus.CREATED) public void createCustomer(
			@RequestBody CreateCustomerRequest request, HttpServletResponse response) {
	...
	@RequestMapping(value = "/courier/createcommand", method = RequestMethod.POST, consumes = MediaType.APPLICATION_JSON_VALUE) @ResponseStatus(value = HttpStatus.CREATED) public void createCourier(
			@RequestBody CreateCourierRequest request, HttpServletResponse response) {
		PersonName personName = new PersonName(request.getFirstName(), request.getLastName());
		CreateCourierCommand command = new CreateCourierCommand(personName, request.getMaxNumberOfActiveOrders(), getAuditEntry());
		commandGateway.send(command, LoggingCallback.INSTANCE);
	}

	@RequestMapping(value = "/restaurant/createcommand", method = RequestMethod.POST, consumes = MediaType.APPLICATION_JSON_VALUE) @ResponseStatus(value = HttpStatus.CREATED) public void createRestaurant(
			@RequestBody CreateRestaurantRequest request, HttpServletResponse response) {
	...
	@RequestMapping(value = "/order/createcommand", method = RequestMethod.POST, consumes = MediaType.APPLICATION_JSON_VALUE) @ResponseStatus(value = HttpStatus.CREATED) public void createOrder(
			@RequestBody CreateOrderRequest request, HttpServletResponse response) {
	...
	@RequestMapping(value = "/restaurant/order/{id}/markpreparedcommand", method = RequestMethod.POST, consumes = MediaType.APPLICATION_JSON_VALUE) @ResponseStatus(value = HttpStatus.CREATED) public void markRestaurantOrderAsPrepared(
			@PathVariable String id, HttpServletResponse response) {
		MarkRestaurantOrderAsPreparedCommand command = new MarkRestaurantOrderAsPreparedCommand(id, getAuditEntry());
		commandGateway.send(command, LoggingCallback.INSTANCE);
	}

	@RequestMapping(value = "/courier/{cid}/order/{oid}/assigncommand", method = RequestMethod.POST, consumes = MediaType.APPLICATION_JSON_VALUE) @ResponseStatus(value = HttpStatus.CREATED) public void assignOrderToCourier(
			@PathVariable String cid, @PathVariable String oid, HttpServletResponse response) {
		AssignCourierOrderToCourierCommand command = new AssignCourierOrderToCourierCommand(oid, cid, getAuditEntry());
		commandGateway.send(command, LoggingCallback.INSTANCE);
	}

	@RequestMapping(value = "/courier/order/{id}/markdeliveredcommand", method = RequestMethod.POST, consumes = MediaType.APPLICATION_JSON_VALUE) @ResponseStatus(value = HttpStatus.CREATED) public void markCourierOrderAsDelivered(
			@PathVariable String id, HttpServletResponse response) {
